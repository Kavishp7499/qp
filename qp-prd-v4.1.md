# Product Requirements Document: `qp` — Quickly Please

**Version:** 0.5.0  
**Status:** Updated to current direction  
**Author:** TBD  
**Last Updated:** 2026-03-23  
**Changelog:** v0.5.0 — removed MCP from the public product surface, made `qp init --docs` and generated `HUMANS.md` / `AGENTS.md` / `CLAUDE.md` core features, added `agent.accrue_knowledge`, broadened `init --from-repo`, and aligned the PRD with the current shipped direction.
...
`qp` is a single interface that exposes how a repository is built, checked, run, and governed.

**`qp.yaml` is the operational API of the repo.** The task manifest, validation pipeline, context document, generated human/agent docs, and agent prompts are all derived from one file. There is no second config, no separate manifest, and no generated code to keep in sync.

`qp` is a repo-agnostic, YAML-configured task runner and AI agent integration tool. It simplifies complex or frequently forgotten commands, orchestrates multi-step workflows with optional parallelism, and makes repository capabilities available to humans and AI coding agents through structured config, generated docs, stable JSON output, and bounded context documents.

The binary is named `qp`. All configuration lives in `qp.yaml` at the repo root.
...
`qp` addresses all of the above with a single config file and a single binary.
...
- Support knowledge accrual back into `qp.yaml` through an explicit agent workflow
...
Engineers who want to standardise how tasks are invoked across a team. They define `qp.yaml` and commit it alongside code, reducing onboarding friction and eliminating "how do I run this?" questions.

### Tertiary: AI Coding Agents

Agents that can read repo files and run local commands. From the agent's perspective, `qp` is the structured interface that explains how the repo works and which commands are safe and relevant.
...
A configurable, explicitly bounded snapshot of the repository for LLM consumption. Default output is short and high-signal. Agent mode (`--agent`) is richer but still bounded by explicit per-section caps. `qp context` is not a generic repo summariser; it exists to answer: *what does an agent need to know to work here right now?*

### Scopes

Named lists of paths declared in `qp.yaml`. Nothing more. Used to constrain what files an agent should touch. Tasks may declare an associated scope by name.

### Prompt

A named text template stored in `qp.yaml` with a fixed, minimal set of interpolation variables. Your prompting strategy lives in the repo, versioned alongside the code.
...
Define named tasks in `qp.yaml`, run them with `qp <task-name>`.

**Cross-platform note:** `qp` is cross-platform, but task commands are whatever the repo author writes. They execute via the system shell (`$SHELL` on Unix, `cmd.exe` on Windows) and are not interpreted or transformed by `qp`. Portability of task commands is the repo author's responsibility. Repos intended for cross-platform use should define portable commands or provide OS-specific task variants. A `check-env` task that verifies required tools are available is a recommended convention for repos with complex toolchains.
...
- `scope` field associates a named scope with this task, used in `qp context --agent`
...
### 6.3 Generated Docs and Knowledge Accrual (`qp init --docs`)

**Priority:** P0 — Core differentiator.

`qp init --docs` generates repo-specific guidance for humans and coding agents directly from `qp.yaml`.
...
- When `agent.accrue_knowledge: true` is set, include explicit instructions telling agents how to propose structured updates back into `qp.yaml`
...
### 6.4 Context and Agent Handoff (`qp context`)
...
- Last cached guard output (`.qp/last-guard.json`) if it exists

**Flags:**

```bash
qp context                            # Default: brief, high-signal summary
qp context --agent --task <n>         # Full agent handoff briefing
qp context --out <file>               # Write to file
qp context --copy                     # Copy to clipboard
qp context --max-tokens <n>           # Global token budget (after per-section caps)
```

---

### 6.5 Guard / Validate (`qp guard [name]`)
...
- Guards are defined under `guards` in `qp.yaml` as named step lists
- `qp guard` runs the default guard; `qp guard <n>` runs a named guard
...
- Last guard result is cached to `.qp/last-guard.json`; only `stderr` per step is cached (stdout is noise for validation reporting)
...
- Defined in `qp.yaml` under `scopes` as named lists of file paths and globs
- `qp scope <n>` prints the path list, one entry per line
- `--format prompt` wraps output as a natural language constraint sentence
- `--json` outputs a JSON object (see [Section 7.4](#74-qp-scope-name---json))
- Referenced in prompt templates via `{{scope.<n>}}`
- Tasks associate a scope by name via the `scope` field: `scope: api`

---

### 6.7 Prompt Templates (`qp prompt <n>`)

**Priority:** P2.

Named prompt templates stored in `qp.yaml`. Versioned alongside the code.
...
- `qp prompt <n>` renders and prints the template to stdout
- `--copy` copies to clipboard
- Unknown variables render unchanged (`{{unknown_var}}`) with a stderr warning; never an error

---

### 6.8 Watch Mode (`qp watch <task>`)

**Priority:** P2.

Re-runs a task on file system changes. Especially useful as `qp watch guard` during agent iteration.

**Requirements:**

- Default watch path: **repo root** — not `context.include`; these are independent concerns and must not be coupled
- `watch.paths` in `qp.yaml` configures default watched paths
...
|---|---|
| `qp list --json` | Array of task descriptors |
| `qp <task> --json` | Task execution result with per-step detail |
| `qp guard --json` | Guard report |
| `qp scope <n> --json` | Scope object with name and path array |
...
# qp is not a secrets manager — this is a convenience for local dev.
...
# --- Agent Behaviour ---
agent:
  accrue_knowledge: true              # generated agent docs instruct agents to propose structured qp.yaml updates
...
```
qp <task>                           Run a named task
qp <task> --dry-run                 Print command without executing
qp <task> --json                    Output result as JSON
qp list                             List all tasks with descriptions
qp list --json                      Task list as JSON
qp init --docs                      Generate HUMANS.md, AGENTS.md, and CLAUDE.md
qp help [task]                      Show help, or detail for a named task
qp --version                        Print qp version (also: qp version)

qp guard [name]                     Run default or named guard
qp guard [name] --json              Guard report as JSON

qp context                          Print repo context (default: brief)
qp context --agent --task <n>       Full agent handoff briefing
qp context --out <file>             Write to file
qp context --copy                   Copy to clipboard
qp context --max-tokens <n>         Global token budget (applied after per-section caps)

qp scope <n>                        Print scope path list (one per line)
qp scope <n> --format prompt        As a natural language constraint sentence
qp scope <n> --json                 As a JSON object

qp prompt <n>                       Render and print a prompt template
qp prompt <n> --copy                Copy to clipboard

qp watch <task>                     Re-run task on file changes
qp watch <task> --path <glob>       Watch specific paths

qp init                             Scaffold a starter qp.yaml and update .gitignore
qp version                          Print qp version (also: qp --version)
```
...
├── internal/
│   ├── config/                   # qp.yaml loading, validation, defaults
...
├── .qp/                         # Runtime state — add to .gitignore
│   └── last-guard.json           # Cached guard result for context --agent
├── qp.yaml                      # qp's own tasks (dogfood)
├── go.mod
└── README.md
```
...
Scaffolds a starter `qp.yaml` in the current directory. Also appends `.qp/` to `.gitignore` (creating the file if it does not exist). The runtime state directory contains ephemeral cached output and must not be committed.

### Key Design Principles

- **`qp.yaml` is the operational API of the repo.** Help text, generated docs, context documents, and JSON schemas are all derived from one file.
- **Shell transparency.** Commands are passed to the system shell as-is. `qp` does not parse or interpret them.
...
- Missing `qp.yaml`: clear message with suggestion to run `qp init`
...
- `qp list` is tabular: task name left-aligned, description after a fixed column separator
...
| 2 | Monorepo: per-subdirectory `qp.yaml` with inheritance? | Strong candidate for v1.1 |
