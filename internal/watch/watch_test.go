...
func TestRunTriggersOnNewFileInWatchedDirectory(t *testing.T) {
	dir := t.TempDir()
	...
	time.Sleep(60 * time.Millisecond)
	if err := os.WriteFile(filepath.Join(dir, "pkg", "new.txt"), []byte("hello"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	case <-done:
	case <-time.After(2 * time.Second):
		t.Fatal("watcher did not trigger after new file creation")
	}
}
