...
func render(out Output) string {
	sections := []string{"# qp agent brief", strings.TrimSpace(out.Context.Markdown)}
	if out.Plan != nil {
...
func suggestedNextStep(out Output) string {
	switch {
	case out.Task != "":
		return fmt.Sprintf("Start with `qp help %s`, then run the task-specific workflow with `qp context --agent --task %s` if you need a narrower brief.", out.Task, out.Task)
