...
func render(out Output) string {
	...
	if len(out.Tasks) > 0 || len(out.Guards) > 0 {
		...
		if len(out.Tasks) > 0 {
			lines = append(lines, "- `qp help "+out.Tasks[0].Name+"`")
			lines = append(lines, "- `qp context --agent --task "+out.Tasks[0].Name+"`")
		}
		if len(out.Guards) > 0 {
			lines = append(lines, "- `qp guard "+out.Guards[0].Name+"`")
		}
	}
