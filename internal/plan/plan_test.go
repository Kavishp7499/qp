...
func TestGenerateMatchesScopesTasksGuardsAndPackages(t *testing.T) {
	...
	cfg := &config.Config{
		...
		Scopes: map[string]config.Scope{
			"cli": {Desc: "CLI work", Paths: []string{"cmd/qp/", "internal/runner/"}},
		},
		Codemap: config.CodemapConfig{
			Packages: map[string]config.CodemapPackage{
				"cmd/qp":          {Desc: "CLI entrypoint"},
				"internal/runner": {Desc: "Execution engine"},
			},
		},
	}

	out, err := Generate(cfg, "/repo", []string{"cmd/qp/main.go"})
	if err != nil {
	...
	if len(out.Packages) == 0 || out.Packages[0].Target != "cmd/qp" {
		t.Fatalf("Packages = %+v, want cmd/qp", out.Packages)
	}
...
func TestGenerateNormalizesAbsolutePaths(t *testing.T) {
	...
	cfg := &config.Config{
		...
		Scopes: map[string]config.Scope{
			"cli": {Paths: []string{"cmd/qp/"}},
		},
	}

	out, err := Generate(cfg, "/repo", []string{filepath.Join("/repo", "cmd/qp/main.go")})
	if err != nil {
		t.Fatalf("Generate() error = %v", err)
	}
...
func TestGenerateFromGitDiff(t *testing.T) {
	...
	writeFile(t, filepath.Join(repoRoot, "cmd/qp/main.go"), "package main\n")
	runGit(t, repoRoot, "add", "cmd/qp/main.go")
