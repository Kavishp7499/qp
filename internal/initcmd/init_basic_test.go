...
func TestRunCreatesStarterFiles(t *testing.T) {
	...
	if !strings.Contains(msg, "created qp.yaml") {
		t.Fatalf("Run() message = %q, want created qp.yaml", msg)
	}

	cfg, err := os.ReadFile(filepath.Join(dir, "qp.yaml"))
	if err != nil {
		t.Fatalf("ReadFile(qp.yaml) error = %v", err)
	}
	if !strings.Contains(string(cfg), "tasks:") {
		t.Fatalf("qp.yaml = %q, want starter tasks", string(cfg))
	}
	if !strings.Contains(string(cfg), "desc: Main CLI commands and closely-related execution packages.") {
		t.Fatalf("qp.yaml = %q, want starter scope description", string(cfg))
	}
	if !strings.Contains(string(cfg), "groups:\n  core:\n") {
		t.Fatalf("qp.yaml = %q, want starter task group", string(cfg))
	}
	if !strings.Contains(string(cfg), "default: check") {
		t.Fatalf("qp.yaml = %q, want starter default task", string(cfg))
	}
	if !strings.Contains(string(cfg), "agent:\n  accrue_knowledge: false\n") {
		t.Fatalf("qp.yaml = %q, want starter agent config", string(cfg))
	}
	...
	if string(gitignore) != ".qp/\n" {
		t.Fatalf(".gitignore = %q, want .qp/ entry", string(gitignore))
	}
}
...
func TestRunIsIdempotent(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "qp.yaml"), []byte(original), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(msg, "qp.yaml already exists") {
	...
	if string(cfg) != original {
		t.Fatalf("qp.yaml changed unexpectedly: %q", string(cfg))
	}
}
...
func TestRunFromRepoInfersGoTasks(t *testing.T) {
	...
	if !strings.Contains(msg, "tip: run `qp init --docs`") {
	...
	if !strings.Contains(got, "project: "+filepath.Base(dir)) {
		t.Fatalf("qp.yaml = %q, want inferred project name", got)
	}
	if !strings.Contains(got, "cmd: go test ./...") {
		t.Fatalf("qp.yaml = %q, want Go test task", got)
	}
	if !strings.Contains(got, "default: check") {
		t.Fatalf("qp.yaml = %q, want inferred default task", got)
	}
	if !strings.Contains(got, "agent:\n  accrue_knowledge: true\n") {
		t.Fatalf("qp.yaml = %q, want inferred agent accrual enabled", got)
	}
	if !strings.Contains(got, "cmd: go build ./...") {
		t.Fatalf("qp.yaml = %q, want Go build task", got)
	}
	if !strings.Contains(got, "steps:\n      - test\n      - build") {
		t.Fatalf("qp.yaml = %q, want check pipeline", got)
	}
	if !strings.Contains(got, "go.mod") {
		t.Fatalf("qp.yaml = %q, want inferred watch paths", got)
	}
}
...
func TestRunFromRepoPrefersMakeTargets(t *testing.T) {
	...
	if !strings.Contains(got, "cmd: make test") {
		t.Fatalf("qp.yaml = %q, want make-backed test task", got)
	}
	if !strings.Contains(got, "default: check") {
		t.Fatalf("qp.yaml = %q, want inferred default task", got)
	}
	if !strings.Contains(got, "cmd: make build") {
		t.Fatalf("qp.yaml = %q, want make-backed build task", got)
	}
	if !strings.Contains(got, "cmd: make check") {
		t.Fatalf("qp.yaml = %q, want make-backed check task", got)
	}
}
...
`
	...
	for _, unwanted := range []string{"hidden:", "_helper:"} {
		if strings.Contains(got, unwanted) {
			t.Fatalf("qp.yaml = %q, did not want private recipe %q", got, unwanted)
		}
	}
}
...
func TestRunFromRepoReadsCapitalizedJustfile(t *testing.T) {
	...
	if !strings.Contains(got, "cmd: just build") {
		t.Fatalf("qp.yaml = %q, want Justfile-backed task", got)
	}
