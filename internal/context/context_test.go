...
func TestGenerateIncludesTaskScopeAndLastGuard(t *testing.T) {
	...
	dir := t.TempDir()
	if err := os.MkdirAll(filepath.Join(dir, ".qp"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(dir, ".qp", "last-guard.json"), []byte(`{"guard":"default"}`), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestGenerateJSONIncludesSectionsAndMarkdown(t *testing.T) {
	...
	if !strings.Contains(out.Markdown, "# qp context") {
