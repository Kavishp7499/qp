...
func TestHandlePayloadInitializeIncludesInstructions(t *testing.T) {
	...
	if !strings.Contains(instructions, "Project: demo") || !strings.Contains(instructions, "qp guard") {
...
func TestToolsExposeSafetyAnnotations(t *testing.T) {
	...
	if annotations["qpSafety"] != "external" {
		t.Fatalf("annotations = %#v, want qpSafety external", annotations)
	}
...
func TestHandlePayloadResourcesList(t *testing.T) {
	...
	repoRoot := t.TempDir()
	if err := os.MkdirAll(filepath.Join(repoRoot, ".qp"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(repoRoot, ".qp", "last-guard.json"), []byte(`{"guard":"default"}`), 0o644); err != nil {
		t.Fatal(err)
	}
	server := New(cfg, repoRoot, runner.New(cfg, repoRoot))
...
func TestHandlePayloadResourcesRead(t *testing.T) {
	...
	resp, notify, err := server.HandlePayload([]byte(`{"jsonrpc":"2.0","id":1,"method":"resources/read","params":{"uri":"qp://scope/cli"}}`), nil)
	if err != nil {
		t.Fatalf("HandlePayload() error = %v", err)
	}
	if notify {
