...
Thanks for taking an interest in `qp`.

## Philosophy

`qp` should stay easy to explain, easy to adopt, and easy for agents to consume. When contributing, prefer changes that reinforce the core model:

- `qp.yaml` is the operational API of the repo
...
CI runs `gofmt`, `go test ./...`, `go vet ./...`, and `go build ./cmd/qp` on pushes to `main` and on pull requests.
