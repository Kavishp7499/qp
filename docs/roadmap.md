...
This roadmap focuses on the major features that would make `qp` feel more complete as a day-to-day task runner and repo interface.

`qp` is intentionally YAML-driven. That is a product choice, not a temporary compromise. The goal is to use a format that is already familiar to teams, easy for tooling to parse, and consistent with the wider ecosystem of config-driven developer tools.
...
- improved importer coverage
  - shipped: Make, just, package.json, Go, Rust, Python, Java, and Docker Compose starters
  - next: broader ecosystem depth and richer per-ecosystem inference
...
- imports or multi-file config
  - useful for larger repos without turning `qp.yaml` into a monolith
...
- schema and editor support
  - shipped: base JSON schema in `qp.schema.json`
