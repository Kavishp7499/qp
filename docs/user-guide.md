...
Manual sync note: expanded manual home page overview.
...
- CEL context includes built-ins like `env(...)`, `branch()`, `tag()`, `profile()`, `param(...)`, `has_param(...)`, `file_exists(...)`, and `os` (`linux`, `darwin`, or `windows`).
