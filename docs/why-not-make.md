...
`make` is ubiquitous, battle-tested, and already present in many repositories. If your repo is well served by a small Makefile and your team is happy with it, there is no need to replace it just to adopt `qp`.

`qp` exists for the cases where a repository needs something a little more explicit than a Makefile alone.
...
`qp` does not win by pretending `make` is bad. It wins when a repo needs a clearer operational interface for both humans and agents.

## Where qp Is Different

### 1. `qp` wraps what you already have

`qp init --from-repo` is designed to sit on top of an existing repo surface.
...
and scaffold a first `qp.yaml` from them.
...
`qp.yaml` is intentionally boring:
...
### 3. `qp` models repo intent, not just commands

A Makefile mostly tells you what can be run.

`qp.yaml` also captures:
...
That is the main difference. `qp` is trying to be the operational API for the repo, not just a command launcher.

### 4. `qp` is designed for agent workflows

The strongest `qp` features are not “run a command.”
...
### 5. `qp` is installable from source in locked-down environments
...
## When qp Is Worth Adding

`qp` is a better fit when:
...
`qp` is not “better than make” in every situation.
...
If your repo needs a clearer interface for both humans and agents, `qp` is the layer you add on top.
