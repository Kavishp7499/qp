...
func runCandidates(args []string, cfg *config.Config) []string {
	candidates := []string{"--json", "--dry-run", "--verbose", "--quiet", "--no-cache", "--allow-unsafe", "--events", "--param", "--var", "--profile", "--no-color"}
	if cfg == nil {
		return candidates
	}
	if len(args) <= 1 {
...
func guardCandidates(args []string, cfg *config.Config) []string {
	candidates := []string{"--json", "--no-cache", "--verbose", "--quiet", "--allow-unsafe", "--events", "--var", "--profile", "--no-color"}
	if cfg != nil {
...
func taskInvocationCandidates(name string, args []string, cfg *config.Config) []string {
	if cfg == nil {
		return []string{"--json", "--dry-run", "--verbose", "--quiet", "--no-cache", "--allow-unsafe", "--events", "--param", "--var", "--profile", "--no-color"}
	}
