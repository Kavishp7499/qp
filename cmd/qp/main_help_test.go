...
func TestRunHelpForTask(t *testing.T) {
	...
	if !strings.Contains(output, "Usage: qp check [--dry-run] [--verbose] [--no-cache] [--allow-unsafe] [--events] [--json]") {
...
`), 0o644); err != nil {
	...
	if !strings.Contains(output, "Usage: qp b [--dry-run] [--verbose] [--no-cache] [--allow-unsafe] [--events] [--json]") {
...
`), 0o644); err != nil {
	...
	output := readStdout()
	for _, want := range []string{"Usage: qp build <target> [<files...>] [--dry-run] [--verbose] [--no-cache] [--allow-unsafe] [--events] [--json]", "positional[1]", "positional[2...]"} {
		if !strings.Contains(output, want) {
			t.Fatalf("stdout = %q, want %q", output, want)
		}
	}
