...
func TestRunHelpForTask(t *testing.T) {
	...
	if !strings.Contains(output, "Usage: qp check [--var name=value] [--profile name] [--dry-run] [--verbose] [--quiet] [--no-cache] [--allow-unsafe] [--events] [--json]") {
...
`), 0o644); err != nil {
	...
	if !strings.Contains(output, "Usage: qp b [--var name=value] [--profile name] [--dry-run] [--verbose] [--quiet] [--no-cache] [--allow-unsafe] [--events] [--json]") {
...
`), 0o644); err != nil {
	...
	output := readStdout()
	for _, want := range []string{"Usage: qp build <target> [<files...>] [--var name=value] [--profile name] [--dry-run] [--verbose] [--quiet] [--no-cache] [--allow-unsafe] [--events] [--json]", "positional[1]", "positional[2...]"} {
		if !strings.Contains(output, want) {
			t.Fatalf("stdout = %q, want %q", output, want)
		}
	}
